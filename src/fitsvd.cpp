...
void Fitsvd::fit() {
    ...
    int i, j;
    double tmp, thresh;
    this->ma = funcs((this->r_star)[0]).size();  // number of basis functions
    ...
    this->solve(b, this->a, thresh);            // solve for the coefficients

    //this->calculate_covar_and_chisq();
}

void Fitsvd::solve(Col<double> b, Col<double> &x, double thresh = -1.0) {
